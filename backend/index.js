...
const sendEmailViaSMTP = async ({ from, to, subject, html, replyTo }) => {
  if (!smtpTransporter) {
    throw new Error('SMTP transporter not configured. Set SMTP_USER and SMTP_PASS in .env.')
  }
  return await withTimeout(
    smtpTransporter.sendMail({ from, to, subject, html, replyTo, priority: 'high' }),
    SEND_TIMEOUT_MS,
    'SMTP send'
  )
}

const sendEmailViaResend = async ({ from, to, subject, html, replyTo }) => {
  if (!resend) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`Send Timeout: ${SEND_TIMEOUT_MS}ms (with 200ms fail-safe buffer)`)
  if (EMAIL_PROVIDER === 'smtp') {
